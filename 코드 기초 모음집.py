...
for : 반복문.
{} : dictionary
- .keys() :키값         ex)dictionary.keys
- .values(): 데이터 값  ex)dictionary.values()
- .items():키 + 데이터  ex)dictionary.items()
(): 튜플
...
f {}: {} 안에 변수나 표현식을 쉽게 삽일할 수 있도록 해준다.
    -f"{변수이름:,}" ->  알아서 숫자를 , 나눠줌
    -f"{변수이름: .2f}" ->소수점 .2 까지 나타내줌
list : 목록 작성에 쓰임
count : 문자열에서 문자가 몇번쨰 나오는지 알려줌 , 리스트에서는 엘리먼트가 몇번쨰있는지 알려줌  ex) 구할 전체 .count("구하고싶은 자리"),몇번 등장하는지 알고싶을때도 쓸수있다
...
del: 리스트안에 있는 요소를 삭제 하거나 리스트전체를 삭제 ex)리스트이름[삭제 인덱스] del은 인덱스로 요소를 삭제하고, remove는 값을 기준으로 요소를 삭제합니다.
...
"\t" : 결과 값을 띄어쓰기 해줌 ,출력을 정렬하고 간격두는데 사용
"\r" : 캐리지 리턴 \r뒤에있는단어를 앞에있는단어에 덮여쓰기   ex)hellohello\rapple" ->applehello
...
set : 중복값을 제거하고 나온다 
zip : 두개의 리스트를 묶어준다 ex)[1,2,3][4,5,6] zip(1,4)(2,5)(3,6)
enumerate : 인덱스와  값을 쌍으로 반환해줌 ex) f= [a,b,c] for i, s in  enumerate(리스트이름,start =1)하면  i = 1이되고 s =리스트의 [0]
<<<<<<< HEAD
슬라이싱 :슬라이싱할 대상 [start:stop:step] ->마지막 stop에 해당되는 인덱스는 포함하지않는다
진법 
2진법 -> 10진법:int(2진수 변수이름,2)
8진법 -> 10진법:int(8진수 변수이름,8)
16진법 ->10진법 int(16진수 변수이름 , 16)
10진법 -> 2진법:bin()
10진법 -> 8진법:oct()
10진법 -> 16진법:hex()

=======
extend : 리스트뒤에 리스트를 붙침 ex)a =[1,2,3] b =[4,5,6] a.extenmd(b) = 1,2,3,4,5,6
zz
